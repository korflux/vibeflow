#!/usr/bin/env bash
# vibe-analyze/scripts/analyze.sh — seleciona uma implementação completa com o mesmo contrato no Unix.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dir) DIR="$2"; shift 2 ;;
    --mvp) MVP=1; shift ;;
    *) echo "uso: analyze.sh [--root DIR] [--apply] [--dir PASTA] [--mvp]" >&2; exit 2 ;;
  esac
done
...
if [[ -n "$PYTHON_BIN" ]]; then
  ...
  [[ -n "$DIR" ]] && args+=(--dir "$DIR")
  [[ "$MVP" -eq 1 ]] && args+=(--mvp)
  exec "$PYTHON_BIN" "$(dirname "$0")/analyze.py" "${args[@]}"
fi
...
if command -v pwsh >/dev/null 2>&1 && pwsh -NoProfile -Command 'exit [int]($PSVersionTable.PSVersion.Major -lt 7)' >/dev/null 2>&1; then
  ...
  [[ -n "$DIR" ]] && args+=(-Dir "$DIR")
  [[ "$MVP" -eq 1 ]] && args+=(-Mvp)
  exec pwsh -NoProfile -ExecutionPolicy Bypass -File "$(dirname "$0")/analyze.ps1" "${args[@]}"
fi

echo "analyze.sh precisa de Python 3 ou PowerShell 7+." >&2
exit 1
