...
# Inventário de .vibeflow/phases → prepara phase-N-slug/analyze.md.
param(
    [string]$Root,
    [switch]$Apply,
    [string]$Dir,
...
function Write-AnalyzeOutput([hashtable]$Payload) {
    ...
    if ($Apply) {
        ...
        try {
            if (New-LiveFile $destFile) {
                $actions.Add([pscustomobject]@{ op = 'criar_arquivo'; alvo = "$rel/analyze.md" })
            }
        } catch {
            throw
        }
