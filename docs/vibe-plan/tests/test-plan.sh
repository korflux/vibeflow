#!/usr/bin/env bash
# Contratos do launcher vibe-plan/scripts/plan.sh: motor, --apply e --dir.
set -u
set -o pipefail
...
LAUNCHER="$HERE/../../../vibe-plan/scripts/plan.sh"
set -e
...
# 3. --dir apontando pasta sem spec devolve PLAN_SEM_SPEC: a flag chegou.
s=$(new_sandbox)
seed_vibeflow "$s"
