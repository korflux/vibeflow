#!/usr/bin/env bash
# vibe-review/scripts/review.sh — seleciona uma implementação completa com o mesmo contrato no Unix.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dir) DIR="$2"; shift 2 ;;
    --mvp) MVP=1; shift ;;
    *) echo "uso: review.sh [--root DIR] [--apply] [--dir PASTA] [--slug SLUG] [--mvp]" >&2; exit 2 ;;
  esac
done
...
if [[ -n "$PYTHON_BIN" ]]; then
  ...
  [[ -n "$DIR" ]] && args+=(--dir "$DIR")
  [[ "$MVP" -eq 1 ]] && args+=(--mvp)
  exec "$PYTHON_BIN" "$(dirname "$0")/review.py" "${args[@]}"
fi
...
if command -v pwsh >/dev/null 2>&1 && pwsh -NoProfile -Command 'exit [int]($PSVersionTable.PSVersion.Major -lt 7)' >/dev/null 2>&1; then
  ...
  [[ -n "$DIR" ]] && args+=(-Dir "$DIR")
  [[ "$MVP" -eq 1 ]] && args+=(-Mvp)
  exec pwsh -NoProfile -ExecutionPolicy Bypass -File "$(dirname "$0")/review.ps1" "${args[@]}"
fi

echo "review.sh precisa de Python 3 ou PowerShell 7+." >&2
exit 1
