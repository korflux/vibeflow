...
# Inventário de .vibeflow/phases → prepara phase-N-slug/review.md.
param(
    [string]$Root,
    [switch]$Apply,
    [string]$Dir,
...
function Write-ReviewOutput([hashtable]$Payload) {
    ...
    if ($Apply) {
        ...
        try {
            if (New-LiveFile $destFile) {
                $actions.Add([pscustomobject]@{ op = 'criar_arquivo'; alvo = "$rel/review.md" })
            }
        } catch {
            if ($createdDir) {
