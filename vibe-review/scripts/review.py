#!/usr/bin/env python3
"""Inventaria .vibeflow/phases e prepara phase-N-slug/review.md."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inventaria e prepara review para .vibeflow/phases")
...
# Inventaria o disco e opcionalmente prepara o artefato vivo review.md.
def run(args: argparse.Namespace) -> Path:
    ...
    if args.mvp:
        ...
        if required:
            raise RuntimeError(f"REVIEW_CADEIA_INCOMPLETA: faltam em .vibeflow/mvp: {', '.join(required)}.")
        alvo = mvp

        modo_sugerido = "atualizar" if "review.md" in mvp["files"] else "reuse"
    ...
    if args.apply:
        ...
        try:
            if prepare_live_file(dest_file):
                actions.append({"op": "criar_arquivo", "alvo": f"{rel}/review.md"})
        except Exception:
            if created_dir:
